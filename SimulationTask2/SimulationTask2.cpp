...
public:
	...
	char transSpeed;
	double clp;
	vector<vector<double> > inifinteQueuePackets;
	double clcq;
...
queue<pair<double , double> > packets;	//Packets which arrive from server
...
void populatepackets(){
	...
	for(int i = 0 ; i < N ; i++){
		...
		packets.push(make_pair (expRandNum * stisq , expRandNum * stcq) );
	}
}
...
void nextMcl(){
	...
	vector<vector<double> > infiniteQueuePackets = sharedObject.inifinteQueuePackets;
	int len = infiniteQueuePackets.size();
	double minDep = INT_MAX;
	for(int i = 0; i < len ; i++){
		if(infiniteQueuePackets[i][0] >= mcl){
			minDep = min(minDep , infiniteQueuePackets[i][0]);
		}
	}
 ...
 */
void performEvent1(){
	double packet = packets.front().first;
	double serviceTime = packets.front().second;
	packets.pop();
	...
	sharedObject.inifinteQueuePackets.push_back(temp);
...
void performEvent2(){
	...
	vector<vector<double> > newInfiniteQueuePackets;
	for(int i = 0 ; i < len ; i++){
		if(sharedObject.inifinteQueuePackets[i][0] == mcl){
			sharedObject.serviceQueue.push_back(sharedObject.inifinteQueuePackets[i][2]);	
		}else{
...
void display(){
	...
	for(int i = 0 ; i < len ; i++){
		...
		vector<vector<double> > q = events[i].inifinteQueuePackets;
...
void displayPackets(){
	...
	queue<pair<double,double> > tmp_q = packets; //copy the original queue to the temporary queue
	int ctr = 1;
	while (!tmp_q.empty())
	{
	    cout<<ctr++<<" -> ("<<tmp_q.front().first<<" , "<<tmp_q.front().second<<")\n";
	    tmp_q.pop();
	} 
...
int main(){
	freopen ("SimulationTask2Output.txt","w",stdout);	//Output is given to this file.
	freopen ("SimulationTask2Input.txt","r",stdin);		//Input is taken from this file.
	input();
	init();
	displayPackets();
	simulate();
